...
from generator.style_extractor import DesignTokens, generate_luma_compat_css
...
def generate_tailwind_source_css_v4(tokens: DesignTokens, vendor: str = "MediaDivision", theme: str = "FTCShopHyva") -> str:
    ...
    Includes universal base styles (typography, buttons, containers) from design tokens.
    Store-specific styles (footer, header, CMS content) are in luma-compat.css.
    """
...
{colors_css}    --font-serif: {font_serif};
    ...
    --screen-lg: 1024px;
    --screen-xl: 1440px;
    --max-width: {tokens.max_width or '1540px'};
    --max-screen: 1920px;
}}
...
def scaffold_hyva_theme(
    ...
    # Generate luma-compat.css from source theme LESS files
    if source_theme_path and os.path.isdir(source_theme_path):
        luma_compat_css = generate_luma_compat_css(source_theme_path)
        compat_path = os.path.join(base, "web", "css", "luma-compat.css")
        os.makedirs(os.path.dirname(compat_path), exist_ok=True)
        with open(compat_path, "w") as f:
            f.write(luma_compat_css)
